...
export function CloudinaryUpload({ onUploaded, accept = "image/*,video/*" }: { onUploaded: (url: string) => void; accept?: string }) {
  ...
  const choose = async (file?: File) => {
    ...
    try {
      ...
      setProgress(100); onUploaded(result.url); toast.success("Upload complete");
  ...
  return <div className={`rounded-md border border-dashed p-3 text-center ${dragging ? "border-primary bg-primary/5" : ""}`} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); void choose(e.dataTransfer.files?.[0]); }}><input ref={inputRef} type="file" accept={accept} className="hidden" onChange={(e) => void choose(e.target.files?.[0])} /><Button type="button" variant="outline" disabled={busy} onClick={() => inputRef.current?.click()}><UploadCloud className="mr-2 h-4 w-4" />{busy ? "Uploading…" : "Upload media"}</Button>{progress > 0 && <Progress value={progress} className="mt-2" />}<p className="mt-1 text-xs text-muted-foreground">Drop an image or video here, or choose a file.</p></div>;
}
