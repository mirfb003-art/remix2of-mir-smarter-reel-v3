alter table public.campaigns add column if not exists channel_mode text not null default 'single';
do $$ begin
  if not exists (select 1 from pg_constraint where conname='campaigns_channel_mode_check') then
    alter table public.campaigns add constraint campaigns_channel_mode_check check (channel_mode in ('single','multi'));
  end if;
end $$;
...
create index if not exists campaign_channel_targets_user_campaign_idx on public.campaign_channel_targets(user_id, campaign_id, is_active);
...
create index if not exists multi_channel_schedules_due_idx on public.multi_channel_schedules(is_active, next_run_at);
...
create index if not exists multi_channel_rounds_campaign_idx on public.multi_channel_rounds(campaign_id, started_at desc);

grant select, insert, update, delete on public.campaign_channel_targets to authenticated;
grant select, insert, update, delete on public.multi_channel_schedules to authenticated;
grant select, insert, update, delete on public.multi_channel_rounds to authenticated;
grant all on public.campaign_channel_targets to service_role;
grant all on public.multi_channel_schedules to service_role;
grant all on public.multi_channel_rounds to service_role;
...
drop policy if exists "campaign channel targets owner" on public.campaign_channel_targets;
create policy "campaign channel targets owner" on public.campaign_channel_targets
  for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
drop policy if exists "multi channel schedules owner" on public.multi_channel_schedules;
create policy "multi channel schedules owner" on public.multi_channel_schedules
  for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
drop policy if exists "multi channel rounds owner" on public.multi_channel_rounds;
create policy "multi channel rounds owner" on public.multi_channel_rounds
  for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());

create trigger trg_cct_updated before update on public.campaign_channel_targets for each row execute function public.set_updated_at();
create trigger trg_mcs_updated before update on public.multi_channel_schedules for each row execute function public.set_updated_at();
